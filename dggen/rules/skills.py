...
def generate_skills(char: Character) -> None:
    ...
    for i in range(char.profession.bonds):
        char.bonds[i] = char.stats["charisma"]
...
def apply_bonuses(
    ...
    while bonuses_applied < number_of_skills_to_boost:
        ...
        boosted = char.skills.get(skill, 0) + boost_by_percentile
        if boosted <= max_skill_level:
            char.skills[skill] = boosted
            bonuses_applied += 1
            ...
            logger.debug(
                "%s, Skipped boost - %s already at %s", char, skill, char.skills.get(skill, 0),
            )
